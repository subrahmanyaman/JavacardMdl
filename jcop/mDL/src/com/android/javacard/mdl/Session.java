...
public class Session {
  ...
  private static final short MAX_HANDOVER_MSG_SIZE = 256;

  static byte[] mBuffer;
  static KeyPair mECp256DeviceKeys;
  static AESKey mDeviceKey;
  static AESKey mReaderKey;
  static HMACKey mHmacKey;
  static Signature mHmacSigner;
  ...
  static byte[] mHandover;
  static ECPrivateKey mDevicePrivateKey;

  static CBORDecoder mDecoder;
  static CBOREncoder mEncoder;
  static short[] mStructure;
  static byte[] mSalt;
  static SEProvider mSEProvider;
  public static void init(){
    ...
    Session.mECp256DeviceKeys = new KeyPair(KeyPair.ALG_EC_FP, KeyBuilder.LENGTH_EC_FP_256);
    Session.mHandover = JCSystem.makeTransientByteArray(MAX_HANDOVER_MSG_SIZE,
    ...
    mSalt = JCSystem.makeTransientByteArray((short) 32, JCSystem.CLEAR_ON_DESELECT);
  }
  ...
  private static void readerKey(byte[] scratchPad, short scratchOffset, short scratchLen){
  ...
  static Signature computeSessionDataAndSecrets(){
    ...
    try {
      ...
      SEProvider.print(mSalt, (short) 0, saltLen);
      SEProvider.print(scratchPad, secretStart, secretLen);
      SEProvider.print(MdlSpecifications.deviceSecretInfo, (short) 0, (short) MdlSpecifications.deviceSecretInfo.length);
      //System.out.println("---------------------------------");

      // Derive SKDevice and SKReader using hkdf.
      outLen = hkdf(mSalt, (short) 0, saltLen, scratchPad, secretStart, secretLen,
          MdlSpecifications.deviceSecretInfo,scratchPad, outStart);
      ...
      outLen = hkdf(mSalt, (short)0, saltLen, scratchPad, secretStart, secretLen,
          MdlSpecifications.readerSecretInfo,scratchPad, outStart);
  ...
  public static short hkdf(byte[] salt, short saltStart, short saltLen,
  ...
  public static short messageDigestSha256(byte[] input, short inputStart, short inputLen,
      byte[] out,
      short outStart){
  ...
  static byte[] getScratchPad(short[] retVal){
   ...
   */
  private static void encodeSessionTranscript(byte[] sessionTranscript, byte[] handoverSelectMsg,
    ...
    if(totalLen > 255){
      sessionTranscript[offset++] = (byte) (MdlSpecifications.CBOR_BINARY_STR | MdlSpecifications.CBOR_UINT16_LENGTH);
      Util.setShort(sessionTranscript, offset, totalLen);
      offset +=2;
    }else {
      sessionTranscript[offset++] = (byte) (MdlSpecifications.CBOR_BINARY_STR | MdlSpecifications.CBOR_UINT8_LENGTH);
  ...
  /**
   *
   * This method incrementally encrypts the data in place. It expects that incoming buffer has
   * at least 16 bytes (one block) extra at end i.e. len = data length + 16;.
   *
   */
  ...
  static short finishIncrementalEncryption(byte[] buf, short start, short len){
    short index = mSEProvider.doAesGcmOperation(buf,start,len,buf,start, false);
    return  (short)(index - start);
  }

  private static short generateNonce(short counter, byte[] scratch, short scratchStart) {
  ...
  public static short coseKey(byte[] buffer, short start, short len, short keyId,
      short[] structure) {
  ...
  public static short copyByteString(byte[] buf, short index, short len, byte[] out, short start){
   ...
   */
  static short generateAndAddEDeviceKey_p256(byte[] buf, short offset){
    ...
    buf[offset++] = (byte)(MdlSpecifications.CBOR_BINARY_STR | MdlSpecifications.CBOR_MAX_256_BYTES);
    ...
    buf[offset++] = 0x02;
    // Write second pair
    buf[offset++] = (byte) (MdlSpecifications.CBOR_NEG_INT | (byte)(1 - 1));
  ...
  static short readAndClearBufferedData(byte[] buf, short start){
